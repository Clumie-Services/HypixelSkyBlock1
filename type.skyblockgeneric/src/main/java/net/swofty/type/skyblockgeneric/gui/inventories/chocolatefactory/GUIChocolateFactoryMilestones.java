    ...
    public void onOpen(InventoryGUIOpenEvent e) {
        ...
        set(new GUIItem(4) {
            ...
            public ItemStack.Builder getItem(HypixelPlayer p) {
                ...
                lore.add("§7Reach chocolate milestones to");
                lore.add("§7unlock special rewards!");
                lore.add("");
                lore.add("§7All-time Chocolate: §6" + StringUtility.commaify(factoryData.getAllTimeChocolate()));
                lore.add("§7Milestones Reached: §a" + reached + "§7/§a" + FactoryMilestone.values().length);

                return ItemStackCreator.getStack("§6Factory Milestones", Material.GOLDEN_APPLE, 1, lore.toArray(new String[0]));
            }
        });

        // Display milestones (slots 10-16, 19-25, 28-34, 37-43)
        int[] slots = {
        ...
        FactoryMilestone[] milestones = FactoryMilestone.values();
        for (int i = 0; i < Math.min(slots.length, milestones.length); i++) {
            FactoryMilestone milestone = milestones[i];
            int slot = slots[i];
            ...
            set(new GUIItem(slot) {
                ...
                public ItemStack.Builder getItem(HypixelPlayer p) {
                    SkyBlockPlayer sbPlayer = (SkyBlockPlayer) p;
                    ChocolateFactoryData factoryData = sbPlayer.getChocolateFactoryData();
                    long playerAllTime = factoryData.getAllTimeChocolate();
                    boolean reached = milestone.isReached(playerAllTime);
                    double progress = milestone.getProgress(playerAllTime);

                    List<String> lore = new ArrayList<>();
                    lore.add("");
                    lore.add("§7Requirement: §6" + ChocolateCalculator.formatNumber(milestone.getChocolateRequired()));
                    lore.add("");
                    lore.add("§7Reward:");
